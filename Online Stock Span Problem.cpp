...
ANS: 
   StockSpanner() {
    ...
    int next(int price) {
        int days=1;
//as soon as we find price in stack less than price coming next we pop s.top() but before that we'll add its days to upcoming greater price's stock.
        while(!s.empty() && s.top().first<=price){
            auto x=s.top();
            s.pop();
            days+=x.second;
            //adding top days to upcoming stock price to get all smaller days before it.
        }
        s.push({price,days});
        return days;
    }
